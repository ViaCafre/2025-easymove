...
import { ServiceOrder, formatCurrency, DriverData } from "../types";

export type PdfRoleTarget = 'motorista' | 'ajudante' | 'montador' | 'embalador' | 'geral';

const cleanPhone = (phone: string) => phone.replace(/\D/g, '');

const PRIMARY_COLOR = "#f59e0b"; // Updated to Brand Orange/Gold
...
const getLogoBase64 = (): Promise<string | null> => {
    return new Promise((resolve) => {
        ...
        img.src = "https://a.imagem.app/Bv6eQJ.png"; // Updated Logo URL
...
const drawHeader = (doc: jsPDF, pageWidth: number, title: string, orderId: string, logoData: string | null) => {
  ...
  doc.text("EASYMOVE TRANSPORTADORA", 50, 20); // Rebranded
...
1. Fui contratado pela empresa EASYMOVE INTERMEDIAÇÃO LOGÍSTICA E TRANSPORTES LTDA, inscrita no CNPJ 54.826.258/0001-70, para realizar o transporte de bens móveis conforme ordem de serviço anexa.

2. Estou ciente de que a EASYMOVE atua apenas como intermediadora logística, não sendo responsável pela execução direta do transporte, sendo este de minha inteira responsabilidade.
...
5. Reconheço que eventuais sinistros deverão ser imediatamente comunicados à EASYMOVE, bem como à autoridade policial competente, caso aplicável.
...
Por ser verdade, firmo o presente termo para que produza seus efeitos legais.`;
  ...
  // Right: Easymove (Digital or Manual)
  doc.line(110, signatureLineY, 175, signatureLineY);
  doc.text("Easymove (Emissor)", 110, signatureLineY + 5);

  // Insert Digital Signature for Easymove if exists
...
// --- GENERIC TEAM PDF GENERATOR (Existing) ---
export const generateTeamPDF = async (order: ServiceOrder, targetRole: PdfRoleTarget, targetName: string, customMessage?: string) => {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  const logoData = await getLogoBase64();
  ...
  // ORIGIN
  doc.setDrawColor(PRIMARY_COLOR); // Updated to use variable which is now Orange
  ...
  if (targetRole === 'motorista') {
      // Fallback in case Generic is used for Driver inadvertently
  ...
  doc.setFillColor(SECONDARY_COLOR);
  doc.roundedRect(20, boxY, pageWidth - 40, 25, 3, 3, "F");

  doc.setTextColor("#94a3b8");
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("VALOR A RECEBER / PAGAMENTO", 30, boxY + 8);
  ...
  doc.setTextColor(PRIMARY_COLOR);
  doc.setFontSize(20);
  doc.text(formatCurrency(paymentAmount), pageWidth - 30, boxY + 16, { align: "right" });

  // 5. SIGNATURE (TEAM/GENERIC)
  const signatureY = boxY + 50;
  ...
  doc.setFontSize(18);
  doc.text("Easymove transportadora LTDA", pageWidth / 2, signatureY - 4, { align: "center" }); // Rebranded

  doc.setTextColor(TEXT_LIGHT);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("Este documento serve como ordem de execução de serviço. Endereços contêm links para mapas.", pageWidth / 2, pageHeight - 12, { align: "center" });
  doc.setFont("helvetica", "bold");
  doc.text("CNPJ: 54.826.258/0001-70   |   Contato: (41) 8747-1778", pageWidth / 2, pageHeight - 7, { align: "center" });
