...
import { mapOrderFromDB, mapOrderToDB, mapTransactionFromDB, mapTransactionToDB } from './src/utils/mappers';
...
function App() {
    ...
    // Remove local storage effect for transactions
    // useEffect(() => {
    ...
    const handleCreate = async (newOrder: ServiceOrder) => {
        ...
        const { error } = await supabase.from('service_orders').insert(dbOrder);
    ...
    const handleUpdate = async (updatedOrder: ServiceOrder) => {
        const dbOrder = mapOrderToDB(updatedOrder);
    ...
    const handleAddTransaction = async (newTransaction: Transaction) => {
        const dbTx = mapTransactionToDB(newTransaction);
    ...
    const filteredOrders = useMemo(() => {
        // If we have a search query, we want to search GLOBALLY, ignoring the current filter tabs.
        if (searchQuery.trim()) {
        ...
        // Otherwise, normal tab logic applies
        ...
        <div className="min-h-screen text-slate-200 font-sans selection:bg-emerald-500/30 pb-32">
                    ...
                    <div className="flex items-center justify-between h-20">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative group cursor-pointer flex items-center justify-center">
                                ...
                                <img
                                    src="https://a.imagem.app/Bv6eQJ.png"
                                    alt="Easymove Logo"
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-emerald-500 font-bold tracking-[0.2em] uppercase">Sistema Easymove Transportadora V1.0</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-4">
                            <div className={`hidden lg:flex items-center gap-3 px-4 py-2 bg-slate-900/50 border rounded-full text-sm text-slate-400 focus-within:text-white transition-all mr-2 ${searchQuery ? 'border-emerald-500/50 ring-1 ring-emerald-500/20' : 'border-white/5 focus-within:border-emerald-500/50 focus-within:ring-1 focus-within:ring-emerald-500/20'}`}>
                                <Search size={16} className={searchQuery ? 'text-emerald-500' : ''} />
                                <input
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="bg-transparent border-none focus:outline-none placeholder-slate-600 w-48 xl:w-64"
                                    placeholder="Buscar por cliente, ID..."
                                />
                                {searchQuery && (
                                    <button onClick={() => setSearchQuery('')} className="p-1 hover:text-white text-slate-500">
                                        <X size={14} />
                                    </button>
                                )}
                            </div>
            ...
            <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                <section className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 transition-opacity duration-500 ${focusedOrderId ? 'opacity-30 blur-[2px] grayscale-[0.5]' : 'opacity-100'}`}>
                    ...
                    <div
                        className="relative overflow-hidden glass-card rounded-2xl p-6 md:col-span-2 group flex flex-col items-center justify-center min-h-[220px] cursor-default transition-all duration-500 hover:shadow-[0_0_40px_-10px_rgba(245,158,11,0.3)] hover:border-emerald-500/50"
                    ...
                    >
                        ...
                        <div className="w-full flex items-center justify-between relative z-10 mb-2">
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_#f59e0b]"></div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-4">
                            <h2 className="text-5xl md:text-7xl font-black tracking-tight flex items-baseline gap-2 filter drop-shadow-[0_0_25px_rgba(245,158,11,0.4)]">
                                <span className="text-transparent bg-clip-text bg-gradient-to-br from-emerald-300 via-emerald-400 to-emerald-500">
                ...
                <section id="orders-list-anchor">
                    <div className={`flex flex-col lg:flex-row items-end justify-between mb-6 gap-6 border-b border-white/5 pb-4 transition-all duration-500 ${focusedOrderId ? 'opacity-50' : 'opacity-100'}`}>
                        <div>
                            ...
                            {searchQuery && (
                                <p className="text-xs text-emerald-400 font-bold mt-1 animate-pulse">
                                    🔍 Exibindo resultados para "{searchQuery}" (Filtros ignorados)
                                </p>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            {/* Filter Group */}
                            <div className={`flex p-1 bg-slate-900/80 backdrop-blur rounded-xl border border-white/5 gap-1 transition-opacity ${searchQuery ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                                {[
                        ...
                        {filteredOrders.length === 0 && (
                            <div className="col-span-full py-20 flex flex-col items-center justify-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/20">
                                ...
                                <p className="text-xs text-slate-600 mt-1">
                                    {searchQuery
                                        ? `Nenhum resultado para "${searchQuery}" em todas as ordens.`
